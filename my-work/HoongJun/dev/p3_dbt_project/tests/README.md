...
## Purpose
Singular (one-off SQL) and custom generic tests that go beyond dbt's built-in `unique`/`not_null`/`relationships`/`accepted_values`. Generic column tests live in the model YAML (`models/marts/_marts.yml`); this folder is for bespoke assertions.

## Needs
- Built staging/marts models.
...
dbt build --select marts       # build + test the marts layer
```
